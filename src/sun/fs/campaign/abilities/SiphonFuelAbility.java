...
public class SiphonFuelAbility extends BaseToggleAbility {
    ...
    protected String getActivationText() {
        if (SUPPLIES_ID != null && getFleet() != null
    ...
    public void createTooltip(TooltipMakerAPI tooltip, boolean expanded) {
            ...
            tooltip.addPara("Increases the range at which the fleet can be detected by %s and consumes one unit of supplies for every %s units of fuel.",
                            ...
                            "" + Misc.getRoundedValueMaxOneAfterDecimal(getFuelPerSupply())
            );
    ...
    protected void applyEffect(float amount, float level) {
        ...
        fleet.getStats().getDetectedRangeMod().modifyPercent(getModId(), DETECTABILITY_PERCENT * level, "Siphoning fuel");
        //fleet.getStats()
        ...
        if (getGasGiant() == null) {
            ...
            fleet.getCargo().removeCommodity(SUPPLIES_ID, cost);
            fleet.getCargo().addCommodity(FUEL_ID, cost * getFuelPerSupply());
        }
    ...
    public CommoditySpecAPI getCommodity() {
            return Global.getSettings().getCommoditySpec(SUPPLIES_ID);
    }
...
//		GL11.glPopMatrix();
		...
		float radStart = getFleet().getRadius() + 5;
...
//		pixelsPerSegment = circ / segments;
		...
		Color color = new Color(255,25,25,195);
